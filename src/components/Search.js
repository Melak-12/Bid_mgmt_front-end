...
const  SearchItem =({searchD})=>{
  ...
  const yo={searchD}["searchD"];
console.log("wwwwwwwwwww",yo,{searchD})
...
const newP=(oldPrice,newPrice,email,id)=>{
  ...
  if(newPrice<=oldPrice){
    ...
    const getData=async(key)=>{
      const result=await fetch('http://127.0.0.1:8000/api/search/'+key);
          const res=await result.json();
          // const da=await data.json();
          console.log('search data is ', res)
          setData(res);
